                        ...
                        <div class="col-sm-10">
                            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type">
                                ...
                                <option value="halaman" @if (old('type') == 'halaman') selected="selected" @endif>Halaman
                                    Statis</option>
                                <option value="artikel" @if (old('type') == 'artikel') selected="selected" @endif>Artikel
                                </option>
                                <option value="program" @if (old('type') == 'program') selected="selected" @endif>Program
                                </option>
                                <option value="layanan" @if (old('type') == 'layanan') selected="selected" @endif>Layanan
                                </option>
                                <option value="rekening" @if (old('type') == 'rekening') selected="selected" @endif>
                                    Rekening Zakat
                                </option>
                                <option value="upz" @if (old('type') == 'upz') selected="selected" @endif>Unit
                                    Pengumpulan Zakat
                                </option>
                                <option value="kalkulator" @if (old('type') == 'kalkulator') selected="selected" @endif>
                                    Kalkulator Zakat
                                </option>
                            </select>
                            @error('type')
...
@push('script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const loadTargets = (type) => {
                if (type === 'artikel') {
                    targetSelect.innerHTML = '<option value="">-- Tidak Perlu Target --</option>';
                    return;
                }
