        ...
        <div class="box box-success">
            <div class="box-header">
                ...
                <div class="pull-right box-tools">
                    <a href="{{ route('artikel.index') }}" class="btn btn-social btn-sm btn-default">
                        <i class="fa fa-reply"></i> Kembali
                    </a>
                </div>
            </div>
            <div class="box-body">
                <form class="form-horizontal" action="{{ route('artikel.update', $artikel->id) }}" method="POST"
                            ...
                            <select class="form-control select2 @error('kategori_id') is-invalid @enderror" id="kategori_id"
                                ...
                                @foreach ($kategoris as $kategori)
                                    <option value="{{ $kategori->id }}"
                                        @if (old('kategori_id', $artikel->kategori_id) == $kategori->id) selected="selected" @endif> {{ $kategori->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('kategori_id')
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <textarea class="tinymce @error('body') is-invalid @enderror" rows="10" name="body" id="body">{{ old('body', $artikel->body) }}</textarea>
                            @error('body')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control @error('status') is-invalid @enderror" id="status"
                                name="status">
                                <option value="" hidden>-- Choose --</option>
                                <option value="0" @if (old('status', $artikel->status) == 0) selected="selected" @endif> Private
                                </option>
                                <option value="1" @if (old('status', $artikel->status) == 1) selected="selected" @endif> Publish
                                </option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">
